...
usage: $(basename "$0" .sh) [-h] [-X value] [-w string] (-d file -v value -o dir -s value)
...
where:
    ...
    -X  Resume mode: specify 0 to start fresh or 1 to resume from medaka polishing.
"

### Terminal Arguments ---------------------------------------------------------

# Import user arguments
while getopts ':hX:d:v:o:s:e:m:M:f:F:r:R:c:p:q:w:n:u:t:T:E:' OPTION; do
  case $OPTION in
    h) echo "$USAGE"; exit 1;;
    X) RESUME_STAGE=$OPTARG;;   # -X takes an argument: 0 (do not resume) or 1 (resume)
...
echo "Maximum expected error rate in raw reads: $MAX_EE"
